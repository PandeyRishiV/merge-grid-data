...
const populate = () => {
  try {
    ...
    const insertRowValue = db.prepare(
      'INSERT INTO row_values (id, "values") VALUES (?, ?)'
    );
    const insertRowRelation = db.prepare(
      "INSERT INTO row_relations (row_id, value_id) VALUES (?, ?)"
    );
    ...
    const initRowValues = db.transaction((values: RowValue[]) => {
      for (const val of values)
        insertRowValue.run(val.id, JSON.stringify(val.values));
    });

    const initRowRelations = db.transaction(
      (rows: Row[], values: RowValue[]) => {
